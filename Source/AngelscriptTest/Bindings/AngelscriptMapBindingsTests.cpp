...
namespace AngelscriptMapBindingsTestPrivate
{
	bool VerifyMapBaseline(
		FAutomationTestBase& Test,
		FAngelscriptEngine& Engine)
	{
		FScopedAngelscriptModule ModuleScope(Test, Engine, TEXT("ASMap_Compat"), ASTEST_AS(R"AS(
	...
	bool VerifyMapTypeMatrix(
		FAutomationTestBase& Test,
		FAngelscriptEngine& Engine)
	{
		FScopedAngelscriptModule ModuleScope(Test, Engine, TEXT("ASMap_TypeMatrix"), ASTEST_AS(R"AS(
			// ---- int → FString ----
	...
	bool VerifyMapApiCoverage(
		FAutomationTestBase& Test,
		FAngelscriptEngine& Engine)
	{
		FScopedAngelscriptModule ModuleScope(Test, Engine, TEXT("ASMap_ApiCoverage"), ASTEST_AS(R"AS(
	...
	bool VerifyMapReturnTypes(
		FAutomationTestBase& Test,
		FAngelscriptEngine& Engine)
	{
		FScopedAngelscriptModule ModuleScope(Test, Engine, TEXT("ASMap_ReturnType"), ASTEST_AS(R"AS(
			// Direct bool returns
	...
	bool VerifyMapFindFailureAndFindOrAddRef(
		FAutomationTestBase& Test,
		FAngelscriptEngine& Engine)
	{
		FScopedAngelscriptModule ModuleScope(Test, Engine, TEXT("ASMap_FindFailure"), ASTEST_AS(R"AS(
	...
	bool VerifyMapLogDiagnostics(
		FAutomationTestBase& Test,
		FAngelscriptEngine& Engine)
	{
		FScopedAngelscriptModule ModuleScope(Test, Engine, TEXT("ASMap_LogDiag"), ASTEST_AS(R"AS(
			int MapLog_Types()
			{
...
using namespace AngelscriptMapBindingsTestPrivate;
	...
	EAutomationTestFlags::EditorContext | EAutomationTestFlags::EngineFilter)
{
	...
	TEST_METHOD(MapBaseline)
	{
		FAngelscriptEngine& Engine = ASTEST_GET_ENGINE();
		FAngelscriptEngineScope Scope(Engine);
		ASSERT_THAT(IsTrue(
			VerifyMapBaseline(*TestRunner, Engine),
			TEXT("VerifyMapBaseline should pass")));
	}

	TEST_METHOD(MapTypeMatrix)
	{
		FAngelscriptEngine& Engine = ASTEST_GET_ENGINE();
		FAngelscriptEngineScope Scope(Engine);
		ASSERT_THAT(IsTrue(
			VerifyMapTypeMatrix(*TestRunner, Engine),
			TEXT("VerifyMapTypeMatrix should pass")));
	}

	TEST_METHOD(MapApiCoverage)
	{
		FAngelscriptEngine& Engine = ASTEST_GET_ENGINE();
		FAngelscriptEngineScope Scope(Engine);
		ASSERT_THAT(IsTrue(
			VerifyMapApiCoverage(*TestRunner, Engine),
			TEXT("VerifyMapApiCoverage should pass")));
	}

	TEST_METHOD(MapReturnTypes)
	{
		FAngelscriptEngine& Engine = ASTEST_GET_ENGINE();
		FAngelscriptEngineScope Scope(Engine);
		ASSERT_THAT(IsTrue(
			VerifyMapReturnTypes(*TestRunner, Engine),
			TEXT("VerifyMapReturnTypes should pass")));
	}

	TEST_METHOD(MapLogDiagnostics)
	{
		FAngelscriptEngine& Engine = ASTEST_GET_ENGINE();
		FAngelscriptEngineScope Scope(Engine);
		ASSERT_THAT(IsTrue(
			VerifyMapLogDiagnostics(*TestRunner, Engine),
			TEXT("VerifyMapLogDiagnostics should pass")));
	}

	TEST_METHOD(MapFindFailureAndFindOrAddRef)
	{
		FAngelscriptEngine& Engine = ASTEST_GET_ENGINE();
		FAngelscriptEngineScope Scope(Engine);
		ASSERT_THAT(IsTrue(
			VerifyMapFindFailureAndFindOrAddRef(*TestRunner, Engine),
			TEXT("VerifyMapFindFailureAndFindOrAddRef should pass")));
	}
};
