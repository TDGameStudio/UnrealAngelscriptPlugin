...
public:
	...
	TEST_METHOD(MathExtended)
	{
		...
		FScopedAngelscriptModule Mod(*TestRunner, Engine, TEXT("ASMathPlatform_MathExtended"), ASTEST_AS(R"AS(
	...
	TEST_METHOD(MathDeterministic)
	{
		...
		FString MathDeterministicSource = ASTEST_AS(R"AS(
			int Entry()
			{
		...
		MathDeterministicSource.ReplaceInline(TEXT("$TOLERANCE$"), *FormatScriptFloatLiteral(Tolerance));
		MathDeterministicSource.ReplaceInline(TEXT("$EXPECTED_CLAMP_ANGLE$"), *FormatScriptFloatLiteral(ExpectedClampAngle));
		MathDeterministicSource.ReplaceInline(TEXT("$EXPECTED_DELTA_ANGLE$"), *FormatScriptFloatLiteral(ExpectedDeltaAngle));
		MathDeterministicSource.ReplaceInline(TEXT("$EXPECTED_UNWIND$"), *FormatScriptFloatLiteral(ExpectedUnwind));
		MathDeterministicSource.ReplaceInline(TEXT("$EXPECTED_VECTOR_INTERP$"), *FormatScriptVectorLiteral(ExpectedVectorInterp));
		MathDeterministicSource.ReplaceInline(TEXT("$EXPECTED_ROTATOR_INTERP$"), *FormatScriptRotatorLiteral(ExpectedRotatorInterp));
		MathDeterministicSource.ReplaceInline(TEXT("$EXPECTED_SCALAR_INTERP$"), *FormatScriptFloatLiteral(ExpectedScalarInterp));

		FAngelscriptEngine& Engine = ASTEST_GET_ENGINE();
		FAngelscriptEngineScope Scope(Engine);

		FScopedAngelscriptModule Mod(*TestRunner, Engine, TEXT("ASMathPlatform_MathDeterministic"), MathDeterministicSource);
		if (!Mod.IsValid()) return;
		auto& M = Mod.GetModule();
	...
	TEST_METHOD(PlatformProcess)
	{
		...
		FScopedAngelscriptModule Mod(*TestRunner, Engine, TEXT("ASMathPlatform_PlatformProcess"), ASTEST_AS(R"AS(
			int UserDirNotEmpty()
			{
				return (!FPlatformProcess::UserDir().IsEmpty()) ? 1 : 0;
			}

			int UserSettingsDirNotEmpty()
			{
				return (!FPlatformProcess::UserSettingsDir().IsEmpty()) ? 1 : 0;
			}

			int UserTempDirNotEmpty()
			{
				return (!FPlatformProcess::UserTempDir().IsEmpty()) ? 1 : 0;
			}

			int ApplicationSettingsDirNotEmpty()
			{
				return (!FPlatformProcess::ApplicationSettingsDir().IsEmpty()) ? 1 : 0;
			}

			int ExecutablePathNotEmpty()
			{
				return (!FPlatformProcess::ExecutablePath().IsEmpty()) ? 1 : 0;
			}

			int ExecutableNameNotEmpty()
			{
				return (!FPlatformProcess::ExecutableName().IsEmpty()) ? 1 : 0;
			}

			int CurrentWorkingDirectoryNotEmpty()
			{
				return (!FPlatformProcess::CurrentWorkingDirectory().IsEmpty()) ? 1 : 0;
			}

			int ComputerNameNotEmpty()
			{
				return (!FPlatformProcess::ComputerName().IsEmpty()) ? 1 : 0;
			}

			int UserNameNotEmpty()
			{
				return (!FPlatformProcess::UserName().IsEmpty()) ? 1 : 0;
			}

			int CanLaunchURLWorks()
			{
				return FPlatformProcess::CanLaunchURL("https://example.com") ? 1 : 0;
			}
			)AS"));
		if (!Mod.IsValid()) return;
		auto& M = Mod.GetModule();
	...
	TEST_METHOD(Logging)
	{
		...
		FScopedAngelscriptModule Mod(*TestRunner, Engine, TEXT("ASMathPlatform_Logging"), ASTEST_AS(R"AS(
