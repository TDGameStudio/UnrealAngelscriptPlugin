	...
	EAutomationTestFlags::EditorContext | EAutomationTestFlags::EngineFilter)
{
	...
	TEST_METHOD(FHitResultDefault)
	{
		...
		FScopedAngelscriptModule Mod(*TestRunner, Engine, TEXT("ASHitResult_Default"), ASTEST_AS(R"AS(
		...
		ASSERT_THAT(IsTrue(
			ExpectGlobalInt(*TestRunner, Engine, Mod.GetModule(), TEXT("int HitResult_DefaultTime()"), TEXT("Default FHitResult constructor sets Time to 1.0"), 1),
	...
	TEST_METHOD(FHitResultDistance)
	{
		FAngelscriptEngine& Engine = ASTEST_GET_ENGINE();
		FAngelscriptEngineScope Scope(Engine);
		FScopedAngelscriptModule Mod(*TestRunner, Engine, TEXT("ASHitResult_Distance"), ASTEST_AS(R"AS(
		...
		ASSERT_THAT(IsTrue(
			ExpectGlobalInt(*TestRunner, Engine, Mod.GetModule(), TEXT("int HitResult_DefaultDistance()"), TEXT("Default FHitResult distance is 0"), 1),
			TEXT("ExpectGlobalInt should pass")));
	}
	...
	TEST_METHOD(FOverlapResultDefault)
	{
		FAngelscriptEngine& Engine = ASTEST_GET_ENGINE();
		FAngelscriptEngineScope Scope(Engine);
		FScopedAngelscriptModule Mod(*TestRunner, Engine, TEXT("ASHitResult_Overlap"), ASTEST_AS(R"AS(
		...
		ASSERT_THAT(IsTrue(
			ExpectGlobalInt(*TestRunner, Engine, Mod.GetModule(), TEXT("int OverlapResult_DefaultNoOverlap()"), TEXT("Default FOverlapResult ItemIndex is 0"), 1),
			TEXT("ExpectGlobalInt should pass")));
	}
};
