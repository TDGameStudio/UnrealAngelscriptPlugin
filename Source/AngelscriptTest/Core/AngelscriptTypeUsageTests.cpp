...
// Core type-usage ownership coverage.
...
TEST_CLASS_WITH_FLAGS(FTypeUsageTests,
...
class UTypeUsageCarrier : UObject
{
	...
	TEST_METHOD(ScriptMembersResolvePrimitiveContainerEnumAndObjectTypes)
	{
		FAngelscriptEngine& Engine = ASTEST_CREATE_ENGINE();
		{ FAngelscriptEngineScope _AutoEngineScope(Engine);
		...
		asIScriptModule* Module = BuildModule(
			*TestRunner,
			Engine,
			"ASTypeUsageFromPropertyScriptMemberTypes",
			TEXT(R"ANGELSCRIPT(
enum EMode
{
	Idle = 3,
...
class FHolder
{
	...
	FPayload Payload;
		...
		if (Module != nullptr)
		{
			...
			asIScriptEngine* ScriptEngine = Engine.GetScriptEngine();
		ASSERT_THAT(IsNotNull(ScriptEngine, TEXT("Script member type test should expose a script engine")));
...
int Produce()
{
	...
	TEST_METHOD(NativePropertiesPreserveInputOutputAndConstQualifiers)
	{
		FAngelscriptEngine& Engine = ASTEST_CREATE_ENGINE();
		{ FAngelscriptEngineScope _AutoEngineScope(Engine);
...
class ATypeUsageNativePropertyProbe : AActor
{
	...
	void Qualifiers(const int&in Input, int&out Output, bool Flag)
	{
		...
		bool bCompiled = false;
		{
			...
			bCompiled = CompileAnnotatedModuleFromMemory(
				&Engine,
				TEXT("ASTypeUsageFromPropertyNativeQualifiers"),
				TEXT("ASTypeUsageFromPropertyNativeQualifiers.as"),
				ScriptSource);
	...
	TEST_METHOD(DataTypesResolveQualifiersHandlesAndContainers)
	{
		FAngelscriptEngine& Engine = ASTEST_CREATE_ENGINE();
		{ FAngelscriptEngineScope _AutoEngineScope(Engine);
		...
		FAngelscriptEngineScope EngineScope(Engine);
		asIScriptEngine* ScriptEngine = Engine.GetScriptEngine();
		ASSERT_THAT(IsNotNull(ScriptEngine, TEXT("Data type qualifier test should expose a script engine")));
		...
		if (ScriptEngine != nullptr)
		{
			...
			ASSERT_THAT(IsNotNull(ActorTypeInfo, TEXT("Data type qualifier test should resolve the native AActor script type")));
