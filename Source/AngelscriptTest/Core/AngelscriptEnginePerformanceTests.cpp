...
static void ResetPerformanceEngineState()
{
...
static TArray<FStartupPerformanceSample> CollectStartupSamples(MeasureFunc&& Measure)
{
...
static FString ValidateAndWriteStartupMetrics(FAutomationTestBase& Test, const FString& RunId, const FString& TestGroup, const TArray<FStartupPerformanceSample>& Samples, const TArray<FString>& Notes)
{
	...
	const bool bMetricsArtifactExists = LocalAssert.IsTrue(
...
static FStartupPerformanceSample MeasureFullStartup()
{
...
static FString MakeShareCleanGeneratedClassName(const FShareCleanWorkload& Workload, const FString& RunSuffix, int32 CycleIndex)
{
	return FString::Printf(TEXT("UASShareCleanCycle_%s_%d_%s"), Workload.Name, CycleIndex, *RunSuffix);
}

static FString FormatReferenceDiagnosticFlags(const UObject* Object)
{
	if (Object == nullptr)
	{
		return TEXT("null");
...
static FString FormatReferenceDiagnosticObject(const UObject* Object, const UObject* TargetObject)
{
	if (Object == nullptr)
	{
		return TEXT("<null>");
...
static FString MakeSingleLineReferenceDiagnostic(FString Value)
{
...
static FString MakeShareCleanScript(const FShareCleanWorkload& Workload, const FString& RunSuffix, int32 CycleIndex)
{
...
static bool CompileShareCleanWorkload(FAngelscriptEngine& Engine, const FShareCleanWorkload& Workload, const FString& RunSuffix, int32 CycleIndex)
{
...
static void CaptureGeneratedClassBeforeReset(FAngelscriptEngine& Engine, const FShareCleanWorkload& Workload, const FString& RunSuffix, int32 CycleIndex, FShareCleanCycleSample& Sample, TWeakObjectPtr<UASClass>& OutWeakGeneratedClass)
{
...
static void CaptureGeneratedClassAfterReset(FShareCleanCycleSample& Sample, const TWeakObjectPtr<UASClass>& WeakGeneratedClass)
{
...
static void CaptureGeneratedClassReferenceDiagnosticsAfterReset(FShareCleanCycleSample& Sample, const TWeakObjectPtr<UASClass>& WeakGeneratedClass)
{
	if (!Sample.bGeneratedClassExpected)
	{
		return;
...
static FShareCleanCycleSample MeasureShareCleanCycle(const FShareCleanWorkload& Workload, const FString& RunSuffix, int32 CycleIndex)
{
...
static void LogShareCleanCycleSample(FAutomationTestBase& Test, const FShareCleanCycleSample& Sample)
{
...
static void AddShareCleanMetric(TArray<FAngelscriptPerformanceMetric>& Metrics, const FString& MetricName, const TArray<double>& Samples, const FString& Unit = TEXT("seconds"))
{
	LogPerformanceMetric(MetricName, Samples);
	Metrics.Add({ MetricName, Samples, ComputeMedian(Samples), Unit, TEXT("AutomationTest") });
}

static void AddShareCleanWorkloadMetrics(TArray<FAngelscriptPerformanceMetric>& Metrics, const FString& WorkloadName, const TArray<FShareCleanCycleSample>& Samples)
{
...
static void AddShareCleanGeneratedClassMetrics(TArray<FAngelscriptPerformanceMetric>& Metrics, const TArray<FShareCleanCycleSample>& Samples)
{
...
static FString ValidateAndWriteShareCleanMetrics(FAutomationTestBase& Test, const TArray<FShareCleanCycleSample>& Samples)
{
