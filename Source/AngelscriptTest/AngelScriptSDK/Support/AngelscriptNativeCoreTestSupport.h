...
namespace AngelscriptNativeTestSupport
{
	...
	{
	public:
		int Write(const void* Pointer, asUINT Size) override
		{
			...
			const int32 WriteOffset = Bytes.Num();
			Bytes.AddUninitialized(static_cast<int32>(Size));
			FMemory::Memcpy(Bytes.GetData() + WriteOffset, Pointer, Size);
			return asSUCCESS;
		}

		int Read(void* Pointer, asUINT Size) override
		{
			if (Pointer == nullptr && Size > 0)
			{
				bReadError = true;
			...
			if (ReadOffset > Bytes.Num() || Size > static_cast<asUINT>(Bytes.Num() - ReadOffset))
			{
				bReadError = true;
				return asERROR;
			}
			...
			if (Size > 0)
			{
				FMemory::Memcpy(Pointer, Bytes.GetData() + ReadOffset, Size);
		...
		// Transitional spelling for existing save/load tests. New code uses
		// ResetReadPosition so all stream state transitions are named consistently.
		...
		{
			Bytes.SetNum(FMath::Clamp(NewSize, 0, Bytes.Num()));
			ReadOffset = FMath::Min(ReadOffset, Bytes.Num());
		}

		void TruncateBy(int32 BytesToRemove)
		{
			Truncate(Bytes.Num() - FMath::Max(BytesToRemove, 0));
		}
		...
		const TArray<asBYTE>& GetBytes() const
		{
			return Bytes;
		}
	...
	inline asIScriptEngine* CreateNativeEngine(FNativeMessageCollector* MessageCollector)
	{
		check(MessageCollector != nullptr);
		...
		const int CallbackResult = ScriptEngine->SetMessageCallback(asFUNCTION(CaptureNativeMessage), MessageCollector, asCALL_CDECL);
		if (CallbackResult < 0)
		{
	...
	inline asIScriptModule* BuildNativeModule(asIScriptEngine* ScriptEngine, const char* ModuleName, const std::string& Source)
	{
		return BuildNativeModule(ScriptEngine, ModuleName, Source.c_str());
	}
	...
	struct FScopedNativeModule
	{
		...
		FScopedNativeModule(
			...
			: FScopedNativeModule(InTest, InEngine, InModuleName, Source.c_str())
		{
		}
	...
	inline asIScriptFunction* GetNativeFunctionByDecl(asIScriptModule* Module, const char* Declaration)
	{
		...
		// A declaration lookup must never silently turn into a name lookup or a
		// sole-function lookup. Such fallback can execute a different overload and
		// make an invalid SDK assertion appear to pass. The fork's raw module
		// parser can reject otherwise canonical declarations (notably normalized
		// const parameters), so retain exactness by comparing the complete public
		// declaration of each module-owned function when the parser path fails.
