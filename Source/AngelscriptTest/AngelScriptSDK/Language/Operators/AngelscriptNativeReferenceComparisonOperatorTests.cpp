...
TEST_CLASS_WITH_FLAGS(FReferenceComparisonOperatorTests,
...
public:
	...
	TEST_METHOD(ReferencesByOperatorRelationAndOrder)
	{
		using namespace AngelscriptNativeTestSupport;
		AS_NATIVE_PRODUCT("LANG-OP-COMPARISON-REFERENCE",
			ENativeEvidence::Compile | ENativeEvidence::Diagnostic | ENativeEvidence::Runtime |
				ENativeEvidence::Metadata | ENativeEvidence::Lifecycle);
		...
		ASSERT_THAT(
			IsNotNull(Engine.Get(), TEXT("reference comparison product should create an engine")));
		...
		FReferenceState State;
		ASSERT_THAT(IsTrue(RegisterReferenceFixtures(*Engine.Get(), State),
			TEXT("reference comparison product should register raw reference fixtures")));
		asITypeInfo* const Root = Engine.Get()->GetTypeInfoByDecl("FComparisonReferenceRoot");
		asITypeInfo* const Derived = Engine.Get()->GetTypeInfoByDecl("FComparisonReferenceDerived");
		asITypeInfo* const Sibling = Engine.Get()->GetTypeInfoByDecl("FComparisonReferenceSibling");
		ASSERT_THAT(IsTrue(Root != nullptr && Derived != nullptr && Sibling != nullptr,
			TEXT("reference comparison fixture types should all publish")));
		...
		{
			for (const FReferenceRelationCase& RelationCase : ReferenceRelationCases)
			{
				for (const FOrderCase& OrderCase : OrderCases)
				{
					const FNativeCaseContext Case(MakeNativeCaseId("LANG-OP-COMPARISON-REFERENCE",
					...
					ASSERT_THAT(IsTrue(bUniqueCaseId,
						*Case.Describe(TEXT("reference comparison case ID should be unique"))));

					const FString ModuleName =
						FString::Printf(TEXT("ASNativeComparisonReference_%hs_%hs_%hs"),
							OperatorCase.CatalogName,
							RelationCase.CatalogName,
							OrderCase.CatalogName);
					const FString Source =
						BuildReferenceComparisonSource(RelationCase, OperatorCase, OrderCase);
					State.Reset();
					int32 BuildResult = asERROR;
					asIScriptModule* Module = CompileReportedSource(
						Engine, *TestRunner, Case.GetId(), ModuleName, Source, BuildResult);
					if (IsReferenceOrdering(OperatorCase))
					{
						ASSERT_THAT(IsTrue(BuildResult < 0,
							*Case.DescribeResult("<reference ordering build>",
								TEXT("negative build result"),
								FString::Printf(TEXT("%d Messages={%s}"),
									BuildResult,
									*Engine.GetMessagesText()))));
						...
						ASSERT_THAT(AreEqual(1,
							Errors.Num(),
							*Case.DescribeResult("<reference ordering diagnostic>",
								TEXT("one causal error"),
								Engine.GetMessagesText())));
						const int32 ExpectedLine =
							LastSourceLineContaining(Source, TEXT("REFERENCE_COMPARE_CAUSE"));
						ASSERT_THAT(IsTrue(ExpectedLine > 0,
							*Case.Describe(
								TEXT("reference ordering source should retain causal line"))));
						if (Errors.Num() == 1)
						{
							ASSERT_THAT(AreEqual(ExpectedLine,
								Errors[0].Row,
								*Case.Describe(
									TEXT("reference ordering error should own operator line"))));
						}
						ASSERT_THAT(AreEqual(0,
							State.Created,
							*Case.Describe(
								TEXT("rejected reference ordering should execute no factory"))));
						ASSERT_THAT(AreEqual(0,
							State.Live,
							*Case.Describe(
								TEXT("rejected reference ordering should retain no object"))));
						if (Module != nullptr)
						{
							ASSERT_THAT(IsNull(GetNativeFunctionByExactDecl(
												   Module, "bool RunReferenceComparison()"),
								*Case.Describe(
									TEXT("rejected ordering should publish no callable entry"))));
						}
						const FTCHARToUTF8 FailedModuleNameUtf8(*ModuleName);
						if (Engine.Get()->GetModule(
								FailedModuleNameUtf8.Get(), asGM_ONLY_IF_EXISTS) != nullptr)
						{
							Engine.Get()->DiscardModule(FailedModuleNameUtf8.Get());
						}

						State.Reset();
						const FString RecoverySource = BuildReferenceRecoverySource();
						...
						ASSERT_THAT(IsTrue(RecoveryBuildResult >= 0,
							*Case.DescribeResult("<reference recovery build>",
								TEXT("successful same-name build"),
								Engine.GetMessagesText())));
						ASSERT_THAT(IsNotNull(Module,
							*Case.Describe(TEXT("reference recovery should publish its module"))));
						if (RecoveryBuildResult < 0 || Module == nullptr)
						{
							return;
						}
						bAllCasesPassed &= ExecuteReferenceRecovery(
							*TestRunner, *Engine.Get(), *Module, Case, State);
					}
					else
					{
						ASSERT_THAT(IsTrue(BuildResult >= 0,
							*Case.DescribeResult("<reference equality build>",
								TEXT("successful build"),
								Engine.GetMessagesText())));
						ASSERT_THAT(IsNotNull(Module,
							*Case.Describe(TEXT("reference equality should publish its module"))));
						ASSERT_THAT(IsTrue(HasNoErrors(Engine.GetMessages()),
							*Case.DescribeResult("<reference equality diagnostics>",
								TEXT("no errors"),
								Engine.GetMessagesText())));
						...
						bAllCasesPassed &= ExecuteReferenceCase(*TestRunner,
							*Engine.Get(),
							*Module,
							Case,
							RelationCase,
					...
					DiscardModule(*TestRunner,
						*Engine.Get(),
						ModuleName,
						Case.Describe(TEXT("reference comparison module")));
				}
			}
		}
		...
		ASSERT_THAT(AreEqual(96,
			ConstructedIds.Num(),
			TEXT("reference comparison product should construct all 96 catalog IDs")));
		ASSERT_THAT(AreEqual(ConstructedIds.Num(),
			UniqueIds.Num(),
			TEXT("reference comparison product should construct no duplicate IDs")));
		ASSERT_THAT(IsTrue(bAllCasesPassed,
			TEXT("every reference comparison cell should satisfy identity, diagnostics, "
				 "evaluation order, lifecycle, recovery, and cleanup")));
	}
};
