...
TEST_CLASS_WITH_FLAGS(FComparisonOperatorTests,
...
public:
	TEST_METHOD(FloatingTypesByOperatorValueAndOrder)
	{
		using namespace AngelscriptNativeTestSupport;
		AS_NATIVE_PRODUCT("LANG-OP-COMPARISON-FLOAT",
		...
		ASSERT_THAT(IsNotNull(
			Engine.Get(), TEXT("floating comparison product should create a standalone engine")));
		...
		ASSERT_THAT(IsTrue(RegisterTraceFunctions(*Engine.Get(), Trace),
			TEXT("floating comparison product should register trace functions")));
		asIScriptContext* const Context = Engine.Get()->CreateContext();
		ASSERT_THAT(IsNotNull(
			Context, TEXT("floating comparison product should create one reusable context")));
		if (Context == nullptr)
		{
			return;
		}
		...
		for (const FFloatTypeCase& TypeCase : FloatTypeCases)
		{
			const FString Type = FloatTypeDeclaration(TypeCase, *Engine.Get());
			const int ExpectedTypeId = PublicFloatTypeId(*Engine.Get(), TypeCase);
			ASSERT_THAT(IsTrue(
				ExpectedTypeId >= 0, TEXT("floating comparison physical type should resolve")));
			for (const FOperatorCase& OperatorCase : OperatorCases)
			{
				const FString ModuleName = FString::Printf(TEXT("ASNativeComparisonFloat_%hs_%hs"),
					TypeCase.CatalogName,
					OperatorCase.CatalogName);
				const FString SourceId = MakeNativeCaseId("LANG-OP-COMPARISON-FLOAT-SOURCE",
					{ANSI_TO_TCHAR(TypeCase.CatalogName), ANSI_TO_TCHAR(OperatorCase.CatalogName)});
				const FString Source =
					BuildFloatComparisonSource(*Engine.Get(), TypeCase, OperatorCase);
				asIScriptModule* const Module =
				...
				for (const FFloatValueCase& ValueCase : FloatValueCases)
				{
					for (const FOrderCase& OrderCase : OrderCases)
					{
						const FNativeCaseContext Case(MakeNativeCaseId("LANG-OP-COMPARISON-FLOAT",
						...
						ASSERT_THAT(IsTrue(bUniqueCaseId,
							*Case.Describe(TEXT("floating comparison case ID should be unique"))));
						if (TypeCase.Type == EFloatType::Float32)
						{
							bAllCasesPassed &= ExecuteFloatCase<float>(*TestRunner,
								*Engine.Get(),
								*Module,
								*Context,
								Case,
						...
						{
							bAllCasesPassed &= ExecuteFloatCase<double>(*TestRunner,
								*Engine.Get(),
								*Module,
								*Context,
								Case,
				...
				DiscardModule(
					*TestRunner, *Engine.Get(), ModuleName, TEXT("floating comparison module"));
			}
		}

		ASSERT_THAT(AreEqual(192,
			ConstructedIds.Num(),
			TEXT("floating comparison product should construct all 192 catalog IDs")));
		ASSERT_THAT(AreEqual(ConstructedIds.Num(),
			UniqueIds.Num(),
			TEXT("floating comparison product should construct no duplicate IDs")));
		ASSERT_THAT(IsTrue(bAllCasesPassed,
			TEXT("every floating comparison cell should satisfy metadata, bits, order, and "
				 "runtime result")));
	}
};
