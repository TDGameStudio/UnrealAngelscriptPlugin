...
TEST_CLASS_WITH_FLAGS(FOverloadedAssignmentConsumerTests,
...
public:
	...
	TEST_METHOD(AssignmentResultsByScenarioAndConsumer)
	{
		using namespace AngelscriptNativeTestSupport;
		AS_NATIVE_PRODUCT("LANG-OP-OVERLOAD-ASSIGNMENT-CONSUMER",
			ENativeEvidence::Compile | ENativeEvidence::Diagnostic | ENativeEvidence::Runtime
				| ENativeEvidence::Metadata | ENativeEvidence::Cleanup);

		RunResultScenarios(
			"LANG-OP-OVERLOAD-ASSIGNMENT-CONSUMER",
			EResultKind::ValueReference,
			AssignmentScenarioSourceCount,
			TEXT("assignment-result overload product"));
	}
};
