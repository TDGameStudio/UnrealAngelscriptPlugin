...
TEST_CLASS_WITH_FLAGS(FAssignmentTargetRejectionTests,
...
public:
	...
	{
		using namespace AngelscriptNativeTestSupport;
		AS_NATIVE_PRODUCT("LANG-OP-ASSIGNMENT-TARGET-REJECTION",
		...
		ASSERT_THAT(
			IsNotNull(Engine.Get(), TEXT("assignment target rejection should create engine")));
		...
		ASSERT_THAT(IsTrue(RegisterFixtures(*Engine.Get(), State, PrimitiveTypes),
			TEXT("assignment target rejection should register no-execution fixtures")));
		...
		ASSERT_THAT(AreEqual(111,
			...
			TEXT("assignment target rejection should retain legal operation types")));

		TArray<FString> ConstructedIds;
		TSet<FString> UniqueIds;
		bool bAllCasesPassed = true;
		for (const FRejectedAssignmentTargetCase& TargetCase : RejectedTargetCases)
		{
			for (const FOperationTypeCase& OperationTypeCase : LegalOperationTypes)
			{
				...
				const FNativeCaseContext Case(
					MakeNativeCaseId("LANG-OP-ASSIGNMENT-TARGET-REJECTION",
						{ANSI_TO_TCHAR(TargetCase.CatalogName), *OperationTypeName}));
				...
				ASSERT_THAT(IsTrue(bUniqueCaseId,
					*Case.Describe(TEXT("assignment target rejection ID should be unique"))));
				const FString ModuleName =
					FString::Printf(TEXT("ASNativeAssignmentTargetReject_%hs_%s"),
						TargetCase.CatalogName,
						*OperationTypeName);
				const FString Source =
					BuildTargetRejectionSource(*Engine.Get(), OperationTypeCase, TargetCase);
				bAllCasesPassed &= VerifyRejectedSource(
					*TestRunner, Engine, Case, ModuleName, Source, "RejectAssignmentTarget", State);
			}
		}
		ASSERT_THAT(AreEqual(222,
			ConstructedIds.Num(),
			TEXT("assignment target rejection should construct all 222 IDs")));
		ASSERT_THAT(AreEqual(ConstructedIds.Num(),
			UniqueIds.Num(),
			TEXT("assignment target rejection should construct no duplicate IDs")));
		ASSERT_THAT(IsTrue(bAllCasesPassed,
			TEXT("every const or temporary assignment rejection should own one diagnostic, "
				 "no execution, same-name recovery, and cleanup")));
	}
};
