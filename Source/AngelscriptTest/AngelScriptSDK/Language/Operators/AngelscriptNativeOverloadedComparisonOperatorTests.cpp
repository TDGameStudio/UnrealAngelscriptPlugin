...
TEST_CLASS_WITH_FLAGS(FOverloadedComparisonOperatorTests,
...
public:
	...
	TEST_METHOD(OverloadsByOperatorRelationOrderAndReceiver)
	{
		using namespace AngelscriptNativeTestSupport;
		AS_NATIVE_PRODUCT("LANG-OP-COMPARISON-OVERLOAD",
		...
		ASSERT_THAT(
			IsNotNull(Engine.Get(), TEXT("overloaded comparison product should create an engine")));
		...
		FOverloadState State;
		ASSERT_THAT(IsTrue(RegisterOverloadFixtures(*Engine.Get(), State),
			TEXT("overloaded comparison product should register lifecycle and trace callbacks")));
		...
		{
			for (const FOverloadRelationCase& RelationCase : OverloadRelationCases)
			{
				for (const FOrderCase& OrderCase : OrderCases)
				{
					...
					{
						const FNativeCaseContext Case(
							MakeNativeCaseId("LANG-OP-COMPARISON-OVERLOAD",
						...
						ASSERT_THAT(IsTrue(bUniqueCaseId,
							*Case.Describe(
								TEXT("overloaded comparison case ID should be unique"))));

						const FString ModuleName =
							FString::Printf(TEXT("ASNativeComparisonOverload_%hs_%hs_%hs_%hs"),
								OperatorCase.CatalogName,
								RelationCase.CatalogName,
								OrderCase.CatalogName,
								ReceiverCase.CatalogName);
						const FString Source = BuildOverloadedComparisonSource(
							OperatorCase, RelationCase, OrderCase, ReceiverCase);
						State.Reset();
						asIScriptModule* const Module =
							CompileAndReport(Engine, *TestRunner, Case.GetId(), ModuleName, Source);
						if (Module == nullptr)
						{
							return;
						}
						bAllCasesPassed &= ExecuteOverloadedComparisonCase(*TestRunner,
							*Engine.Get(),
							*Module,
							Case,
							OperatorCase,
						...
						DiscardModule(*TestRunner,
							*Engine.Get(),
							ModuleName,
							Case.Describe(TEXT("overloaded comparison module")));
					}
				}
			}
		}
		...
		ASSERT_THAT(AreEqual(96,
			ConstructedIds.Num(),
			TEXT("overloaded comparison product should construct all 96 catalog IDs")));
		ASSERT_THAT(AreEqual(ConstructedIds.Num(),
			UniqueIds.Num(),
			TEXT("overloaded comparison product should construct no duplicate IDs")));
		ASSERT_THAT(IsTrue(bAllCasesPassed,
			TEXT("every overloaded comparison cell should satisfy exact resolution, bytecode, "
				 "marker, value, lifecycle, and cleanup evidence")));
	}
};
