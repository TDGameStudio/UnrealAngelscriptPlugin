...
TEST_CLASS_WITH_FLAGS(FIncrementTypeRejectionTests,
...
public:
	...
	TEST_METHOD(BoolTypeRejections)
	{
		using namespace AngelscriptNativeTestSupport;
		AS_NATIVE_PRODUCT("LANG-OP-INCREMENT-TYPE-REJECTION",
		...
		ASSERT_THAT(IsNotNull(Engine.Get(), TEXT("bool increment rejection should create engine")));
		...
		ASSERT_THAT(IsTrue(CollectTypes(NumericTypes, BoolType),
			TEXT("bool increment rejection should resolve primitive types")));
		...
		ASSERT_THAT(IsTrue(RegisterFixtures(*Engine.Get(), State, NumericTypes, *BoolType),
			TEXT("bool increment rejection should register writable shapes")));
		asITypeInfo* const BoolProperty =
			Engine.Get()->GetTypeInfoByDecl("FIncrementProperty_bool");
		ASSERT_THAT(
			IsNotNull(BoolProperty, TEXT("bool increment property owner should publish metadata")));
		if (BoolProperty != nullptr)
		{
			asIScriptFunction* Getter =
				BoolProperty->GetMethodByDecl("bool get_Value() const");
			if (Getter == nullptr)
			{
				Getter = BoolProperty->GetMethodByName("get_Value");
			}
			asIScriptFunction* Setter = BoolProperty->GetMethodByDecl("void set_Value(bool)");
			if (Setter == nullptr)
			{
				Setter = BoolProperty->GetMethodByName("set_Value");
			}
			ASSERT_THAT(IsTrue(Getter != nullptr && Setter != nullptr,
				TEXT("bool rejection property owner should retain explicit accessors")));
			if (Getter != nullptr && Setter != nullptr && (!Getter->IsProperty() || !Setter->IsProperty()))
			{
				TestRunner->AddInfo(TEXT("[AS-FORK-LIMITATION] bool increment property decorator is unavailable; explicit accessors retained"));
			}
		}
		...
		for (const FCategoryCase& CategoryCase : CategoryCases)
		{
			for (const FOperatorCase& OperatorCase : OperatorCases)
			{
				const FNativeCaseContext Case(MakeNativeCaseId("LANG-OP-INCREMENT-TYPE-REJECTION",
				...
				ASSERT_THAT(IsTrue(bUniqueCaseId,
					*Case.Describe(TEXT("bool increment rejection ID should be unique"))));
				const FString ModuleName =
					FString::Printf(TEXT("ASNativeIncrementBoolReject_%hs_%hs"),
						CategoryCase.CatalogName,
						OperatorCase.CatalogName);
				const FString Source = BuildBoolRejectionSource(OperatorCase, CategoryCase);
				bAllCasesPassed &= VerifyRejectedSource(
					*TestRunner, Engine, Case, ModuleName, Source, "RejectIncrementBool", State);
			}
		}
		ASSERT_THAT(AreEqual(16,
			ConstructedIds.Num(),
			TEXT("bool increment rejection should construct all 16 IDs")));
		ASSERT_THAT(AreEqual(ConstructedIds.Num(),
			UniqueIds.Num(),
			TEXT("bool increment rejection should construct no duplicate IDs")));
		ASSERT_THAT(IsTrue(bAllCasesPassed,
			TEXT("every writable bool rejection should own one diagnostic, no accessor "
				 "execution, metadata, same-name recovery, and cleanup")));
	}
};
