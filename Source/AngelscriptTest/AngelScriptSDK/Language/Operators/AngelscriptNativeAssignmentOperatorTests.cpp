...
TEST_CLASS_WITH_FLAGS(FAssignmentOperatorTests,
...
public:
	TEST_METHOD(TypesByOperatorAndCategory)
	{
		using namespace AngelscriptNativeTestSupport;
		AS_NATIVE_PRODUCT("LANG-OP-ASSIGNMENT",
		...
		ASSERT_THAT(
			IsNotNull(Engine.Get(), TEXT("assignment product should create standalone engine")));
		if (Engine.Get() == nullptr)
		{
			return;
		}
		const TArray<const FNativeTypeCase*> PrimitiveTypes = CollectPrimitiveTypes();
		ASSERT_THAT(AreEqual(11,
			PrimitiveTypes.Num(),
			TEXT("assignment product should resolve all eleven primitive types")));
		if (PrimitiveTypes.Num() != 11)
		{
			return;
		}
		FAssignmentState State;
		ASSERT_THAT(IsTrue(RegisterFixtures(*Engine.Get(), State, PrimitiveTypes),
			TEXT("assignment product should register raw SDK properties and observers")));
		const TArray<FOperationTypeCase> LegalOperationTypes =
			CollectOperationTypeCases(PrimitiveTypes, true);
		ASSERT_THAT(AreEqual(111,
			LegalOperationTypes.Num(),
			TEXT("assignment product should retain all 111 type-supported forms")));
		if (LegalOperationTypes.Num() != 111)
		{
			return;
		}
		asIScriptContext* const Context = Engine.Get()->CreateContext();
		ASSERT_THAT(
			IsNotNull(Context, TEXT("assignment product should create one reusable context")));
		if (Context == nullptr)
		{
			return;
		}
		...
		{
			for (const FOperationTypeCase& OperationTypeCase : LegalOperationTypes)
			{
				const FString OperationTypeName = OperationTypeCase.GetCatalogName();
				const FNativeCaseContext Case(MakeNativeCaseId("LANG-OP-ASSIGNMENT",
				...
				ASSERT_THAT(IsTrue(bUniqueCaseId,
					*Case.Describe(TEXT("assignment legal case ID should be unique"))));
			}
		}
		...
		for (const FNativeTypeCase* TypeCase : PrimitiveTypes)
		{
			for (const FAssignmentCategoryCase& CategoryCase : CategoryCases)
			{
				const FString ModuleName = FString::Printf(TEXT("ASNativeAssignment_%hs_%hs"),
					TypeCase->CatalogName,
					CategoryCase.CatalogName);
				asIScriptModule* const Module =
					CompileLegalSource(Engine, *TestRunner, *TypeCase, CategoryCase, ModuleName);
				if (Module == nullptr)
				{
					return;
				}
				for (const FOperationTypeCase& OperationTypeCase : LegalOperationTypes)
				{
					if (OperationTypeCase.Type != TypeCase)
					{
						continue;
					}
					const FString OperationTypeName = OperationTypeCase.GetCatalogName();
					const FNativeCaseContext Case(MakeNativeCaseId("LANG-OP-ASSIGNMENT",
						{ANSI_TO_TCHAR(CategoryCase.CatalogName), *OperationTypeName}));
					bAllCasesPassed &= ExecuteLegalCase(*TestRunner,
						*Engine.Get(),
						*Module,
						*Context,
						Case,
				...
				ASSERT_THAT(AreEqual(asSUCCESS,
					Engine.Get()->DiscardModule(ModuleNameUtf8.Get()),
					TEXT("assignment module should discard")));
				ASSERT_THAT(
					IsNull(Engine.Get()->GetModule(ModuleNameUtf8.Get(), asGM_ONLY_IF_EXISTS),
						TEXT("assignment module should leave no stale owner")));
			}
		}
		ASSERT_THAT(AreEqual(444,
			ConstructedIds.Num(),
			TEXT("assignment product should construct all 444 legal IDs")));
		ASSERT_THAT(AreEqual(ConstructedIds.Num(),
			UniqueIds.Num(),
			TEXT("assignment product should construct no duplicate legal IDs")));
		ASSERT_THAT(IsTrue(bAllCasesPassed,
			TEXT("every legal assignment cell should prove result, final state, one write, "
				 "metadata, ownership, context reuse, and cleanup")));
	}
};
