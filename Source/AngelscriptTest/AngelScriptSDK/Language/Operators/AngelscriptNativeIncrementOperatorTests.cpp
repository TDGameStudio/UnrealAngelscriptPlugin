...
TEST_CLASS_WITH_FLAGS(FIncrementOperatorTests,
...
public:
	TEST_METHOD(TypesByOperatorCategoryAndObservation)
	{
		using namespace AngelscriptNativeTestSupport;
		AS_NATIVE_PRODUCT("LANG-OP-INCREMENT",
		...
		ASSERT_THAT(
			IsNotNull(Engine.Get(), TEXT("increment product should create standalone engine")));
		...
		ASSERT_THAT(IsTrue(CollectTypes(NumericTypes, BoolType),
			TEXT("increment product should resolve ten numeric types and bool")));
		...
		ASSERT_THAT(IsTrue(RegisterFixtures(*Engine.Get(), State, NumericTypes, *BoolType),
			TEXT("increment product should register property and rejection fixtures")));
		asIScriptContext* const Context = Engine.Get()->CreateContext();
		ASSERT_THAT(
			IsNotNull(Context, TEXT("increment product should create one reusable context")));
		if (Context == nullptr)
		{
			return;
		}
		...
		for (const FCategoryCase& CategoryCase : CategoryCases)
		{
			for (const FObservationCase& ObservationCase : ObservationCases)
			{
				for (const FOperatorCase& OperatorCase : OperatorCases)
				{
					for (const FNativeTypeCase* TypeCase : NumericTypes)
					{
						const FNativeCaseContext Case(MakeNativeCaseId("LANG-OP-INCREMENT",
						...
						ASSERT_THAT(IsTrue(bUniqueCaseId,
							*Case.Describe(TEXT("increment case ID should be unique"))));
					}
				}
			}
		}
		...
		for (const FNativeTypeCase* TypeCase : NumericTypes)
		{
			for (const FCategoryCase& CategoryCase : CategoryCases)
			{
				const FString ModuleName = FString::Printf(TEXT("ASNativeIncrement_%hs_%hs"),
				...
				ASSERT_THAT(AreEqual(asSUCCESS,
					Engine.Get()->DiscardModule(ModuleNameUtf8.Get()),
					TEXT("increment module should discard")));
				ASSERT_THAT(
					IsNull(Engine.Get()->GetModule(ModuleNameUtf8.Get(), asGM_ONLY_IF_EXISTS),
						TEXT("increment module should leave no stale owner")));
			}
		}

		ASSERT_THAT(AreEqual(480,
			ConstructedIds.Num(),
			TEXT("increment product should construct all 480 legal observation IDs")));
		ASSERT_THAT(AreEqual(ConstructedIds.Num(),
			UniqueIds.Num(),
			TEXT("increment product should construct no duplicate IDs")));
		ASSERT_THAT(IsTrue(bAllCasesPassed,
			TEXT("every legal increment cell should satisfy exact before/result/after, "
				 "one-write, metadata, lifecycle, and cleanup evidence")));
	}
};
