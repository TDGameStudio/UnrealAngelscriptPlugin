...
TEST_CLASS_WITH_FLAGS(FConstructorsTests, "Angelscript.TestModule.AngelScriptSDK.Language.Constructors", EAutomationTestFlags::EditorContext | EAutomationTestFlags::EngineFilter)
{
private:
...
int Entry()
{
	...
	return Value.Value + 2;
		...
		ASSERT_THAT(IsNotNull(Context, TEXT("Constructor-chain test should create a context")));
		if (Function == nullptr || Context == nullptr) return;
		ON_SCOPE_EXIT { Context->Release(); };
		ASSERT_THAT(AreEqual(static_cast<int32>(asEXECUTION_EXCEPTION), PrepareAndExecute(Context, Function), TEXT("The current isolated native engine should expose the known constructor-chain execution limitation")));
		ASSERT_THAT(AreEqual(FString(TEXT("Null pointer access")), FString(UTF8_TO_TCHAR(Context->GetExceptionString())), TEXT("Constructor-chain limitation should preserve the current fork exception text")));
	}
