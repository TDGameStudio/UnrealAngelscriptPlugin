...
namespace AngelscriptCompilerBuilderIntegrationTests_Private
{
	...
	bool ContainsMessageFragment(const TArray<FString>& Messages, const TCHAR* Fragment)
	{
	...
	bool ContainsDiagnosticFragment(const TArray<FAngelscriptCompileTraceDiagnosticSummary>& Diagnostics, const TCHAR* Fragment)
	{
...
int Entry()
{
	return BuilderIntegrationAdd(2);
		...
		{
			const int32 PhaseIndex = AngelscriptCompilerBuilderIntegrationTests_Private::FindFirstPhaseIndex(Events, Phase);
		...
		const FAngelscriptCompilationEvent* CodeEvent = AngelscriptCompilerBuilderIntegrationTests_Private::FindFirstEvent(Events, EAngelscriptCompilationEventType::CompileModuleCompileCode);
...
int Entry()
{
	...
	return 42;
		...
		ASSERT_THAT(IsTrue(
			AngelscriptCompilerBuilderIntegrationTests_Private::ContainsDiagnosticFragment(Summary.Diagnostics, TEXT("GhostBuilderType")),
			TEXT("Builder integration invalid script should capture unknown-type diagnostic")));

		const FAngelscriptCompilationEvent* EndEvent = AngelscriptCompilerBuilderIntegrationTests_Private::FindFirstEvent(Events, EAngelscriptCompilationEventType::CompileEnd);
		if (this->Assert.IsNotNull(EndEvent, TEXT("Builder integration invalid script should emit Compile.End")))
		{
			...
			ASSERT_THAT(IsTrue(
				EndEvent->DiagnosticCount > 0 || AngelscriptCompilerBuilderIntegrationTests_Private::ContainsMessageFragment(EndEvent->Messages, TEXT("GhostBuilderType")),
