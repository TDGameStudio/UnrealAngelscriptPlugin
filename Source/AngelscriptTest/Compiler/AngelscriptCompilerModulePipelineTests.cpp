...
TEST_CLASS_WITH_FLAGS(FCompilerModulePipelineTests,
...
public:
	...
	TEST_METHOD(CompilerModulePipelineVariableScopes)
	{
		FAngelscriptEngine& Engine = ASTEST_CREATE_ENGINE();
		{ FAngelscriptEngineScope _AutoEngineScope(Engine);
		asIScriptModule* Module = BuildModule(
...
int Entry()
{
	...
	TEST_METHOD(CompilerModulePipelineFunctionCalls)
	{
		FAngelscriptEngine& Engine = ASTEST_CREATE_ENGINE();
		{ FAngelscriptEngineScope _AutoEngineScope(Engine);
		asIScriptModule* Module = BuildModule(
	...
	TEST_METHOD(CompilerModulePipelineTypeConversions)
	{
		FAngelscriptEngine& Engine = ASTEST_CREATE_ENGINE();
		{ FAngelscriptEngineScope _AutoEngineScope(Engine);
		asIScriptModule* Module = BuildModule(
	...
	TEST_METHOD(NegativeAndFloat64ConversionsProduceExecutableBytecode)
	{
		FAngelscriptEngine& Engine = ASTEST_CREATE_ENGINE();
		{ FAngelscriptEngineScope _AutoEngineScope(Engine);
		asIScriptModule* Module = BuildModule(
			*TestRunner,
			Engine,
			"CompilerTypeConversionsNegativeAndFloat64Conversions",
		...
		ASSERT_THAT(AreEqual(709, Result, TEXT("Compiler type conversion test should truncate both float32 negatives and float64 positives toward zero")));

		asUINT BytecodeLength = 0;
		asDWORD* Bytecode = Function->GetByteCode(&BytecodeLength);
		ASSERT_THAT(IsNotNull(Bytecode, TEXT("Compiler type conversion test should expose generated bytecode")));
		ASSERT_THAT(IsTrue(BytecodeLength > 0, TEXT("Compiler type conversion test should emit at least one bytecode instruction")));
