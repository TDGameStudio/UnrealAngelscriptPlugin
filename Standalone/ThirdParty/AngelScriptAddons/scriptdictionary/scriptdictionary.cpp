...
struct SDictionaryCache
{
	...
	static void Setup(asIScriptEngine *engine)
	{
		...
		if( cache == 0 )
		{
			cache = static_cast<SDictionaryCache*>(asAllocMem(sizeof(SDictionaryCache)));
			if (cache == 0)
				throw std::bad_alloc();
			new(cache) SDictionaryCache;
...
void RegisterScriptDictionary_Native(asIScriptEngine *engine)
{
	...
	r = engine->RegisterObjectMethod("dictionaryValue", "dictionaryValue &opAssign(float64)", asFUNCTIONPR(CScriptDictValue_opAssign, (double, CScriptDictValue*), CScriptDictValue &), asCALL_CDECL_OBJLAST); assert( r >= 0 );
	...
	r = engine->RegisterObjectMethod("dictionaryValue", "float64 opConv()", asFUNCTIONPR(CScriptDictValue_opConvDouble, (CScriptDictValue*), double), asCALL_CDECL_OBJLAST); assert( r >= 0 );
	...
	r = engine->RegisterObjectMethod("dictionary", "void set(const string &in, const float64&in)", asMETHODPR(CScriptDictionary,Set,(const dictKey_t&,const double&),void), asCALL_THISCALL); assert( r >= 0 );
	r = engine->RegisterObjectMethod("dictionary", "bool get(const string &in, float64&out) const", asMETHODPR(CScriptDictionary,Get,(const dictKey_t&,double&) const,bool), asCALL_THISCALL); assert( r >= 0 );
...
void RegisterScriptDictionary_Generic(asIScriptEngine *engine)
{
	...
	r = engine->RegisterObjectMethod("dictionaryValue", "dictionaryValue &opAssign(float64)", asFUNCTION(CScriptDictValue_opAssign_double_Generic), asCALL_GENERIC); assert( r >= 0 );
	...
	r = engine->RegisterObjectMethod("dictionaryValue", "float64 opConv()", asFUNCTION(CScriptDictValue_opConvDouble_Generic), asCALL_GENERIC); assert( r >= 0 );

	r = engine->RegisterObjectType("dictionary", sizeof(CScriptDictionary), asOBJ_REF | asOBJ_GC | asOBJ_IMPLICIT_HANDLE); assert( r >= 0 );
	r = engine->RegisterObjectBehaviour("dictionary", asBEHAVE_FACTORY, "dictionary f()", asFUNCTION(ScriptDictionaryFactory_Generic), asCALL_GENERIC); assert( r>= 0 );
	// The upstream generic list-factory ABI is intentionally not exposed
	// through the maintained fork until a dedicated bridge is provided.
	...
	r = engine->RegisterObjectMethod("dictionary", "void set(const string &in, const float64&in)", asFUNCTION(ScriptDictionarySetFlt_Generic), asCALL_GENERIC); assert( r >= 0 );
	r = engine->RegisterObjectMethod("dictionary", "bool get(const string &in, float64&out) const", asFUNCTION(ScriptDictionaryGetFlt_Generic), asCALL_GENERIC); assert( r >= 0 );
	...
	// Support foreach
	r = engine->RegisterObjectType("dictionaryIter", 0, asOBJ_REF | asOBJ_IMPLICIT_HANDLE); assert(r >= 0);
