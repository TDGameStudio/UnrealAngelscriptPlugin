...
static void *DefaultArrayAlloc(size_t size, size_t)
{
	return asAllocMem(size);
}

static asALLOCFUNC_t userAlloc = DefaultArrayAlloc;
...
{
	// Allocate the memory
	void *mem = userAlloc(sizeof(CScriptArray), alignof(CScriptArray));
	if( mem == 0 )
	{
...
static void RegisterScriptArray_Native(asIScriptEngine *engine)
{
	...
	r = engine->RegisterObjectType("array<class T>", 0, asOBJ_REF | asOBJ_GC | asOBJ_TEMPLATE | asOBJ_IMPLICIT_HANDLE); assert( r >= 0 );
...
void CScriptArray::Reserve(asUINT maxElements)
{
	...
	SArrayBuffer *newBuffer = reinterpret_cast<SArrayBuffer*>(userAlloc(sizeof(SArrayBuffer)-1 + elementSize*maxElements, alignof(SArrayBuffer)));
	if( newBuffer )
	{
...
void CScriptArray::Resize(int delta, asUINT at)
{
	...
	{
		// Allocate memory for the buffer
		SArrayBuffer *newBuffer = reinterpret_cast<SArrayBuffer*>(userAlloc(sizeof(SArrayBuffer)-1 + elementSize*(buffer->numElements + delta), alignof(SArrayBuffer)));
		if( newBuffer )
		{
...
{
	*buf = reinterpret_cast<SArrayBuffer*>(userAlloc(sizeof(SArrayBuffer)-1+elementSize*numElements, alignof(SArrayBuffer)));
...
void CScriptArray::Precache()
{
	...
	// Create the cache
	cache = reinterpret_cast<SArrayCache*>(userAlloc(sizeof(SArrayCache), alignof(SArrayCache)));
	if( !cache )
	{
		asIScriptContext *ctx = asGetActiveContext();
		if( ctx )
...
static void RegisterScriptArray_Generic(asIScriptEngine *engine)
{
	...
	r = engine->RegisterObjectType("array<class T>", 0, asOBJ_REF | asOBJ_GC | asOBJ_TEMPLATE | asOBJ_IMPLICIT_HANDLE); assert( r >= 0 );
	r = engine->RegisterObjectBehaviour("array<T>", asBEHAVE_TEMPLATE_CALLBACK, "bool f(int&in Type, int&out ErrorMessage)", asFUNCTION(ScriptArrayTemplateCallback_Generic), asCALL_GENERIC); assert( r >= 0 );

	r = engine->RegisterObjectBehaviour("array<T>", asBEHAVE_FACTORY, "array<T> f(int&in Type)", asFUNCTION(ScriptArrayFactory_Generic), asCALL_GENERIC); assert( r >= 0 );
	r = engine->RegisterObjectBehaviour("array<T>", asBEHAVE_FACTORY, "array<T> f(int&in Type, uint length)", asFUNCTION(ScriptArrayFactory2_Generic), asCALL_GENERIC); assert( r >= 0 );
	r = engine->RegisterObjectBehaviour("array<T>", asBEHAVE_FACTORY, "array<T> f(int&in Type, uint length, const T &in value)", asFUNCTION(ScriptArrayFactoryDefVal_Generic), asCALL_GENERIC); assert( r >= 0 );
	// The maintained fork's list-factory ABI has one initialization-buffer
	// parameter and injects template metadata outside the declaration. The
	// upstream generic wrapper expects two declared parameters, so list
	// initialization stays unavailable until a dedicated bridge is provided.
