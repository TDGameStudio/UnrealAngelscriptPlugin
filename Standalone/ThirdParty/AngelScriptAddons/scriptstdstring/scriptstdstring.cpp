...
typedef std::unordered_map<
	...
	FScriptStringHash,
	std::equal_to<string>,
	TScriptStdAllocator<std::pair<const string, int>>> map_t;
END_AS_NAMESPACE
#else
#include <map>      // std::map
BEGIN_AS_NAMESPACE
...
static const asPWORD STD_STRING_FACTORY_CACHE = 1004;

static void CleanupStdStringFactory(asIScriptEngine* engine)
{
	auto* stringFactory = static_cast<CStdStringFactory*>(
		engine->SetUserData(0, STD_STRING_FACTORY_CACHE));
	if (stringFactory != 0)
	{
		assert(stringFactory->stringCache.empty());
		stringFactory->~CStdStringFactory();
		asFreeMem(stringFactory);
	}
}

static CStdStringFactory* GetStdStringFactory(asIScriptEngine* engine)
{
	auto* stringFactory = static_cast<CStdStringFactory*>(
		engine->GetUserData(STD_STRING_FACTORY_CACHE));
	if (stringFactory == 0)
	{
		void* memory = asAllocMem(sizeof(CStdStringFactory));
		if (memory == 0)
			throw std::bad_alloc();
		stringFactory = new(memory) CStdStringFactory();
		engine->SetUserData(stringFactory, STD_STRING_FACTORY_CACHE);
		engine->SetEngineUserDataCleanupCallback(
			CleanupStdStringFactory,
			STD_STRING_FACTORY_CACHE);
	}
...
static string AddStringToString(const string& Left, const string& Right)
{
	return Left + Right;
}

template<typename T>
static void AppendValueToString(string& Destination, const T& Value)
{
...
#define AS_STRING_FORMAT_IMPL(tid, type) \
	case tid: AppendValueToString(result, *(type*)ref); break
				...
				default:
					...
					else // enums
					{
						...
						AppendValueToString(result, *(int*)ref);
					}
				}
			}
		}
...
#define AS_STRING_SCAN_IMPL(tid, type) \
	...
	break
		...
		else if (typeId == engine->GetStringFactoryReturnTypeId())
		{
			string val;
			ss >> val;
			if (!ss) goto end_scan;
...
void RegisterStdString_Native(asIScriptEngine *engine)
{
	...
	r = engine->RegisterStringFactory("string", GetStdStringFactory(engine));
...
//	r = engine->RegisterObjectMethod("string", "bool isEmpty() const", asMETHOD(string, empty), asCALL_THISCALL); assert( r >= 0 );
	...
	r = engine->RegisterObjectMethod("string", "string &opAssign(float64)", asFUNCTION(AssignDoubleToString), asCALL_CDECL_OBJLAST); assert( r >= 0 );
	r = engine->RegisterObjectMethod("string", "string &opAddAssign(float64)", asFUNCTION(AddAssignDoubleToString), asCALL_CDECL_OBJLAST); assert( r >= 0 );
	r = engine->RegisterObjectMethod("string", "string opAdd(float64) const", asFUNCTION(AddStringDouble), asCALL_CDECL_OBJFIRST); assert( r >= 0 );
	r = engine->RegisterObjectMethod("string", "string opAdd_r(float64) const", asFUNCTION(AddDoubleString), asCALL_CDECL_OBJLAST); assert( r >= 0 );

	r = engine->RegisterObjectMethod("string", "string &opAssign(float32)", asFUNCTION(AssignFloatToString), asCALL_CDECL_OBJLAST); assert( r >= 0 );
	r = engine->RegisterObjectMethod("string", "string &opAddAssign(float32)", asFUNCTION(AddAssignFloatToString), asCALL_CDECL_OBJLAST); assert( r >= 0 );
	r = engine->RegisterObjectMethod("string", "string opAdd(float32) const", asFUNCTION(AddStringFloat), asCALL_CDECL_OBJFIRST); assert( r >= 0 );
	r = engine->RegisterObjectMethod("string", "string opAdd_r(float32) const", asFUNCTION(AddFloatString), asCALL_CDECL_OBJLAST); assert( r >= 0 );
	...
	// The maintained parser does not expose upstream variadic registration
	// declarations. Fixed-signature format/parse helpers below remain available.
	...
	r = engine->RegisterGlobalFunction("string formatFloat(float64 val, const string &in options = \"\", uint width = 0, uint precision = 0)", asFUNCTION(formatFloat), asCALL_CDECL); assert(r >= 0);
	...
	r = engine->RegisterGlobalFunction("float64 parseFloat(const string &in, uint &out byteCount = 0)", asFUNCTION(parseFloat), asCALL_CDECL); assert(r >= 0);
...
void RegisterStdString_Generic(asIScriptEngine *engine)
{
...
#if AS_NO_IMPL_OPS_WITH_STRING_AND_PRIMITIVE == 0
	// Automatic conversion from values
	r = engine->RegisterObjectMethod("string", "string &opAssign(float64)", asFUNCTION(AssignDouble2StringGeneric), asCALL_GENERIC); assert( r >= 0 );
	r = engine->RegisterObjectMethod("string", "string &opAddAssign(float64)", asFUNCTION(AddAssignDouble2StringGeneric), asCALL_GENERIC); assert( r >= 0 );
	r = engine->RegisterObjectMethod("string", "string opAdd(float64) const", asFUNCTION(AddString2DoubleGeneric), asCALL_GENERIC); assert( r >= 0 );
	r = engine->RegisterObjectMethod("string", "string opAdd_r(float64) const", asFUNCTION(AddDouble2StringGeneric), asCALL_GENERIC); assert( r >= 0 );

	r = engine->RegisterObjectMethod("string", "string &opAssign(float32)", asFUNCTION(AssignFloat2StringGeneric), asCALL_GENERIC); assert( r >= 0 );
	r = engine->RegisterObjectMethod("string", "string &opAddAssign(float32)", asFUNCTION(AddAssignFloat2StringGeneric), asCALL_GENERIC); assert( r >= 0 );
	r = engine->RegisterObjectMethod("string", "string opAdd(float32) const", asFUNCTION(AddString2FloatGeneric), asCALL_GENERIC); assert( r >= 0 );
	r = engine->RegisterObjectMethod("string", "string opAdd_r(float32) const", asFUNCTION(AddFloat2StringGeneric), asCALL_GENERIC); assert( r >= 0 );
	...
	r = engine->RegisterGlobalFunction("string formatFloat(float64 val, const string &in options = \"\", uint width = 0, uint precision = 0)", asFUNCTION(formatFloat_Generic), asCALL_GENERIC); assert(r >= 0);
	...
	r = engine->RegisterGlobalFunction("float64 parseFloat(const string &in, uint &out byteCount = 0)", asFUNCTION(parseFloat_Generic), asCALL_GENERIC); assert(r >= 0);
}
